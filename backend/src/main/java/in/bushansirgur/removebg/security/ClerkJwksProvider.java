package in.bushansirgur.removebg.security;
...
public class ClerkJwksProvider {
    ...
    public PublicKey getPublicKey(String kid) throws Exception {
        if(keyCache.containsKey(kid) && System.currentTimeMillis() - lastFetchTime < CACHE_TTL){ // ← CACHE_TTl → CACHE_TTL, add {
            return keyCache.get(kid); // ← return the cached key
        }
        refreshKeys(); // ← refresh if not cached
        return keyCache.get(kid); // ← return after refresh
    }
